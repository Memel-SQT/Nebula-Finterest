...
- Files modified: `package.json`, `index.html`, `.gitignore`, `build/installer.nsh`, `build/icon.ico`, `build/icon.png`, `assets/app-icon.svg`, `assets/icon.png`, `scripts/fix-windows-icon.js`, `src/electron/main.ts`, `src/renderer/theme.ts`, `src/renderer/styles.css`, `src/renderer/main.tsx`, `src/renderer/i18n.ts`, `src/renderer/App.tsx`, `src/renderer/components/SplashScreen.tsx`, `src/renderer/components/AccountGate.tsx`, `src/renderer/components/SettingsPanel.tsx`, `README.md`, `CLAUDE.md`, `DEV_CHANGES.md`, `PATCH_NOTES.md`.
- **Adopted the Nebula design language and renamed the product to "Nebula Finterest"**, per request. The DA was not invented: it was lifted from the sibling projects on this machine — `Nebula/src/nebula/web/static/css/theme.css` is the canonical source (its tokens are also ported 1:1 into `Nebula News/tailwind.config.ts`), so palette, the `linear-gradient(100deg, blue, violet)` accent, the drifting `.bg-glow`, and the `--motion-fast/base` + `cubic-bezier(0.4,0,0.2,1)` motion scale all match the family exactly rather than approximating it.
- `theme.ts`: `Theme` went from `light | dark | system` to `nebula-dark | nebula-light | old-dark | old-light | system`. The previous emerald/gold palettes are preserved verbatim under `[data-theme='old-dark'] / ['old-light']` (kept selectable, frozen), and `system` now resolves to the Nebula pair. Added a `LEGACY_THEMES` migration so the `'light'`/`'dark'` values already in users' `localStorage` land on the new identity instead of silently falling back to `system`.
- `styles.css` restructured: shared non-color tokens (radii, easings, motion durations, accent gradient) on a bare `:root`, then four palette blocks. `--accent-gradient` resolves per-theme through `var()`, and the `old-*` blocks flatten it to a plain color and hide `.bg-glow` — so the old themes render exactly as before, without gradients or ambient glow. Category colors were remapped into the Nebula palette (blue/violet/amber/success) with a distinct rose for danger, since Nebula's own sheet has no error color.
- New logo (`assets/nebula-logo.svg` + 1024px `assets/app-icon.svg`): Nebula's gradient-on-dark-plate treatment applied to a finance mark — orbit ellipse, three ascending bars, and a star that happens to complete an "i". Regenerated `build/icon.png`, `assets/icon.png` and the multi-resolution `build/icon.ico` (16→256) from it with `@resvg/resvg-js` + `png-to-ico` installed `--no-save`, then uninstalled; note the newer `png-to-ico` exports on `.default` rather than as a bare function.
- `SplashScreen.tsx` rebuilt around the new mark with a staged sequence (plate scale-in → breathing halo → orbit stroke-draw → three bars growing from their base → star spring-pop → wordmark → sweeping progress bar), 2400 ms, still collapsing to 250 ms under `prefers-reduced-motion`. The bars need `transform-box: view-box` with viewBox-coordinate `transform-origin`s, otherwise they scale from the wrong point.
- Animation pass across the app: `sidebar-in`, `rise-in` for the topbar/cards/panels with a 40 ms stagger capped at five cards, hover/active feedback on nav items and list rows. Deliberately restrained — one infinite animation on screen (the glow), everything else is an entrance.
- **Data-safety work around the rename.** Electron derives `userData` from `productName`, so renaming would have pointed every existing install at an empty `%APPDATA%\Nebula Finterest`. `main.ts` now pins `app.setPath('userData', <appData>/Finterest)` before `AccountManager` is constructed (BudgetStore resolves its path at construction time, so ordering matters). Checked the consequences on uninstall against the real template (`app-builder-lib/templates/nsis/uninstaller.nsh`): `deleteAppDataOnUninstall` already removes `$APPDATA\${APP_PACKAGE_NAME}` — package.json `name` is `finterest`, which matches the pinned folder case-insensitively on Windows — so no extra macro was needed. An earlier draft added a `customUnInstall` doing `RMDir /r "$APPDATA\Finterest"`; that was removed after reading the template, because `customUnInstall` runs on **updates** too and would have deleted user data on every upgrade.
- `artifactName` was changed from `${productName}-Setup-${version}` to the literal `Nebula-Finterest-Setup-${version}` — with a two-word productName the template would have reintroduced spaces in the filename, i.e. exactly the updater bug diagnosed in Session #37. `scripts/fix-windows-icon.js` now reads the exe name from `build.productName` instead of hard-coding `Finterest.exe`. `build/installer.nsh` updated for the new exe and backup folder names.
- `NEBULA_DESIGN.md` written as a standalone reference sheet for AI agents (principles, both palettes, gradient/glow/motion recipes, component recipes, the logo SVG and its animation timeline, and an explicit "à ne pas faire" list), then added to `.gitignore` per request — it is shared out of band, not versioned.
- Validation: `npm run typecheck`, `npm run lint`, `npm test -- --runInBand`, `npm run build` all pass. Smoke-tested in the Vite dev server against a temporary IPC stub: verified all four themes resolve their tokens correctly (including the gradient flattening and `.bg-glow` being hidden on `old-*`), and walked the splash → account grid → welcome → login → dashboard → settings flow in both Nebula themes. Caught and fixed one real regression that way: removing `background: var(--page)` from `.account-gate` (needed so the glow shows through) left the viewport unpainted below short content, fixed by painting `html` and switching `min-height: 100%` to `100vh`.
- Not verified here, as usual: the real Electron run, the installed-app icon, and the userData pin under a packaged build — this environment cannot launch unsigned executables.

## [2026-08-23] - Finterest Change Session #41
- Files modified: `scripts/fix-windows-icon.js`, `DEV_CHANGES.md`.
- Publishing v0.1.34 surfaced a real bug in Session #39's icon-fix automation: the repackaging step shelled out to `npx electron-builder ...` via `execFileSync('npx.cmd', [...])`, which fails on Windows with `EINVAL` — `execFileSync` without `shell: true` can't invoke `.cmd` batch-file wrappers directly. (The rcedit step before it, a plain `.exe`, worked fine and did patch the icon; only the repackaging half of the script failed.)
- Fixed by switching to electron-builder's programmatic API (`require('electron-builder').build({ targets, prepackaged, config })`, using `createTargets([Platform.WINDOWS], 'nsis')`) instead of spawning a CLI subprocess at all — sidesteps the Windows `.cmd` issue entirely and avoids the alternative fix (`shell: true`) that Node warns can be an argument-injection risk.
- Ran `npm run dist:win` twice from a clean state to confirm the fix is solid, not just a one-off: both times the icon patch and repackage completed without manual intervention. Verified the final `Finterest-Setup-0.1.34.exe` and `win-unpacked/Finterest.exe` both carry the correct icon via `Icon.ExtractAssociatedIcon`.
- Published the `v0.1.34` GitHub Release (guest session, icon fix, macOS/Linux CI standby) with the corrected build; verified both asset URLs resolve (HTTP 302) before considering it done, per the process lesson from Session #37.
- Validation: `npm run typecheck`, `npm run lint` pass. `npm run dist:win` succeeds end-to-end (verified twice).

## [2026-08-23] - Finterest Change Session #40
- Files modified: `src/shared/accounts.ts`, `src/shared/types.ts`, `src/electron/accounts.ts`, `src/electron/store.ts`, `src/electron/main.ts`, `src/electron/preload.ts`, `src/shared/global.d.ts`, `src/renderer/i18n.ts`, `src/renderer/components/AccountGate.tsx`, `src/renderer/components/ProfileScreen.tsx`, `src/renderer/components/SettingsPanel.tsx`, `src/renderer/App.tsx`, `DEV_CHANGES.md`, `PATCH_NOTES.md`.
- **Added a guest session**, per request ("une session invité... pas save, juste un compte invité par défaut"): `BudgetStore` (`store.ts`) gained an `ephemeral` constructor option — when set, `initialize()` never reads an existing file (always starts from a fresh in-memory `sql.js` database) and `persist()` is a no-op, so nothing ever touches disk. `AccountManager.enterGuestMode(name)` (`accounts.ts`) creates one of these and sets it active with a synthetic record (`id: GUEST_ACCOUNT_ID`, a new shared constant in `src/shared/accounts.ts` so both the main process and renderer can check identity without importing across the process boundary) that is never pushed into `this.records` or written to `accounts.json` — `list()`, the account-management screen, and `exportAllForBackup()` (the pre-uninstall backup) all naturally exclude it since they only ever see `this.records`.
- New `account:guest` IPC handler, exposed as `window.finterest.enterGuestMode(name)`. Entry points added to `AccountGate.tsx`: an "Essayer sans compte" button on both the `select` stage (existing accounts) and the `create` stage (true first launch, no accounts yet) — calls the existing seed-data path (`ensureSeedData()`, same starter budget already used for new accounts) so a guest immediately sees a populated example rather than an empty screen.
- Guarded the one guest action that would otherwise touch disk: `setActiveAvatar` now throws a new `ERR_GUEST_READONLY` code (mapped to a translated message) if the active account is the guest, since it copies a file into `userData/avatars/`. `renameActive` needed no such guard — it already only calls `saveRecords()` (which serializes `this.records`, and the guest was never added to it), so renaming the in-memory guest display name was already inherently safe, just skipped the save call defensively for clarity.
- `ProfileScreen.tsx` and `SettingsPanel.tsx` detect the guest account (`account.id === GUEST_ACCOUNT_ID`) and swap their normal controls for a plain-language notice ("vos modifications ne sont pas enregistrées...") — hides the photo/pseudonym editing and export/import buttons, and turns "Changer de compte" into "Quitter le mode invité" (same underlying handler; exiting just calls the existing `lockAccount()` + reload flow, which discards the ephemeral store since nothing referenced it once `this.active` is cleared).
- Validation: `npm run typecheck`, `npm run lint`, `npm test -- --runInBand`, `npm run build` all pass. Clicked "Essayer sans compte" via the Vite dev server (no Electron bridge available there) and confirmed it fails gracefully through the existing error-banner path rather than crashing — the actual `enterGuestMode` IPC round-trip and in-memory persistence behavior still need verification in the real packaged/dev Electron app, which this environment cannot drive.

## [2026-08-23] - Finterest Change Session #39
- Files added: `scripts/fix-windows-icon.js`.
- Files modified: `package.json`, `.github/workflows/build-installers.yml`, `DEV_CHANGES.md`, `PATCH_NOTES.md`.
- **Actually fixed the desktop/Explorer/Start Menu icon** (previously documented as a known limitation requiring Developer Mode). Key discovery: the `winCodeSign` package that failed to extract (`Cannot create symbolic link`, Session #37) only fails on its bundled **macOS** dylib symlinks — the Windows `rcedit-x64.exe` inside the same archive extracts fine regardless, and several copies were already sitting in `%LOCALAPPDATA%\electron-builder\Cache\winCodeSign\*\rcedit-x64.exe` from earlier failed attempts. Ran `rcedit-x64.exe win-unpacked\Finterest.exe --set-icon build\icon.ico` directly (bypassing electron-builder's `signAndEditExecutable` gate entirely, so no Developer Mode / elevation needed), then repackaged the NSIS installer from that already-patched folder via `electron-builder --win nsis --prepackaged <dir>` (skips re-packing, just rebuilds the installer). Verified via `Icon.ExtractAssociatedIcon` in both `win-unpacked/Finterest.exe` and the final `Finterest-Setup-0.1.33.exe` — both now show the real F monogram, not Electron's default.
- Automated this as `scripts/fix-windows-icon.js`, appended to the end of the `dist:win` npm script so every future build gets the fix automatically: it locates a cached `rcedit-x64.exe` (falls back to downloading a standalone copy from `github.com/electron/rcedit` releases, which has no symlinks and always extracts cleanly), patches the icon, and re-invokes electron-builder with `--prepackaged`. No-ops safely on non-Windows hosts or if rcedit can't be obtained. Ran the full `npm run dist:win` pipeline from a clean state to confirm the automation works end-to-end, not just as a one-off manual fix.
- Replaced the v0.1.33 GitHub Release's installer/`latest.yml` assets in place (again) with this icon-corrected build — same tag, no version bump, consistent with the artifact-naming fix precedent from Session #37.
- **Put macOS and Linux CI builds on standby**, per explicit request: removed the `macos` and `linux` entries from `.github/workflows/build-installers.yml`'s build matrix (commented explaining why and how to re-add them), leaving only `windows`. Left `package.json`'s `dist:mac`/`dist:linux` npm scripts and `build.mac`/`build.linux` electron-builder config untouched — a developer can still build them manually on their native platform; only the automatic CI builds (which were producing unsigned, unpublished, unused artifacts) are paused.
- Validation: `npm run typecheck`, `npm run lint` pass. `npm run dist:win` (with the new icon-fix step) succeeds end-to-end from a clean build.

## [2026-08-23] - Finterest Change Session #38
- Files removed: `USER_UPDATE_SUMMARY.txt`.
- Files modified: `README.md`, `.github/copilot-instructions.md`, `CLAUDE.md`.
- Per explicit user request ("1 readme et un patch note clair"), removed `USER_UPDATE_SUMMARY.txt` — it duplicated `PATCH_NOTES.md`'s content for end users at a different verbosity, and the user wanted exactly one user-facing changelog. Confirmed with the user first that `DEV_CHANGES.md` (the technical/developer log, not styled as end-user patch notes and relied on for cross-session continuity) should stay.
- Updated the mandatory-documentation contract (`.github/copilot-instructions.md`) to require three files instead of four (`README.md`, `DEV_CHANGES.md`, `PATCH_NOTES.md`), folded the "plain language, step-by-step" guidance that used to live in the removed section 5.4 into section 5.3's `PATCH_NOTES.md` description, and fixed the remaining `USER_UPDATE_SUMMARY.txt` references (checklist, backup-requirements section). Updated `CLAUDE.md`'s doc-convention line to match.
- Re-verified `README.md` for accuracy while doing this: fixed the installer filename example (`Finterest Setup x.x.x.exe` → `Finterest-Setup-x.x.x.exe`, matching the artifact-naming fix from Session #37) and removed the now-dead link to `USER_UPDATE_SUMMARY.txt`. The rest of the feature list, installation, usage, and developer sections were checked against current app behavior and found accurate — no other changes needed.
- No `PATCH_NOTES.md` entry added for this change — it's a repository documentation reshuffle with no effect on the packaged app, consistent with the judgment call made in Session #34 for the README.txt removal.
- Validation: `npm run typecheck` and `npm run lint` pass (no source files changed).

## [2026-08-23] - Finterest Change Session #37
- Files modified: `package.json`, `DEV_CHANGES.md`, `PATCH_NOTES.md`, `USER_UPDATE_SUMMARY.txt`.
- **Found the real cause of "Impossible de vérifier les mises à jour."** reported by the user on the installed v0.1.32 build. First ruled out the repo being private (`gh repo view` confirms `PUBLIC`). The actual bug: electron-builder's NSIS default `artifactName` template produces `Finterest Setup 0.1.33.exe` (with spaces), but `latest.yml` (also generated by electron-builder) references the artifact as `Finterest-Setup-0.1.33.exe` (hyphens) — the two naming schemes disagree internally. Since the release was published by manually uploading the raw build output via `gh release create` rather than through electron-builder's own `--publish` flow, GitHub additionally mangled the spaces into dots on upload (`Finterest.Setup.0.1.33.exe`), so the actual hosted asset name matched neither the local filename nor what `latest.yml` pointed at — `electron-updater`'s download step 404'd against a URL that could never exist under any of these three names.
- Fixed by adding `"artifactName": "${productName}-Setup-${version}.${ext}"` to `package.json`'s top-level `build` config, so the locally-built file now matches what `latest.yml` expects exactly (confirmed: `install/windows/Finterest-Setup-0.1.33.exe`, no spaces, matches `latest.yml`'s `url:`/`path:` fields byte-for-byte).
- Rebuilt with this fix and replaced the v0.1.33 GitHub Release's mismatched assets in place (`gh release delete-asset` + `gh release upload`, same tag — no new version needed since this is a packaging-config fix, not an app behavior change). Verified both asset URLs now resolve (HTTP 302, not 404) via `curl -I` against the exact URLs electron-updater constructs.
- v0.1.32's already-published release asset has the same naming mismatch and was left as-is — nothing needs to download *from* it now that v0.1.33 supersedes it, so fixing it has no user-facing benefit; flagged here in case a future session revisits old releases.
- Lesson for future releases: always verify a freshly published release's asset name matches `latest.yml`'s `url`/`path` fields (e.g. `curl -I` the exact download URL) before telling the user to test the update — this was not done for v0.1.32 or the first upload of v0.1.33, which is how the bug shipped twice before being caught.

## [2026-08-23] - Finterest Change Session #36
- Files modified: `src/renderer/styles.css`, `src/renderer/components/AccountGate.tsx`, `src/renderer/App.tsx`.
- **Found and fixed a real layout bug**: `.account-card > button { width: 100%; ... }` used a direct-child combinator, but a `.account-stage` wrapper `<div>` (added in Session #31 for the stage-fade-in animation) had been inserted between `.account-card` and every button since — so every primary button on the account gate (Créer et continuer, Continuer, Se connecter, Ajouter un compte, Retour, Gérer les comptes...) silently stopped stretching full width and lost its margin-top, likely the direct cause of the "boutons parfois trop proches du texte" the user reported. Changed the selector to `.account-stage > button`, verified via `getBoundingClientRect()` that buttons now correctly fill the card's content width again.
- General spacing pass on top of that fix, since several other gaps were tighter than they should be: `.field-grid` gap 0.8rem→0.9rem and its button margin-top 0.15rem→0.5rem, `.row-actions`/`.settings-actions`/`.profile-actions`/`.manage-confirm`/`.calendar-form` gaps all bumped ~0.15-0.2rem, `.account-card label`/`> button` margins increased.
- **Darkened the light theme** (was reported as too bright): `--page` #f6f7f5→#eceeea, `--surface` #ffffff→#fbfbf9 (no more pure white), `--surface-raised`/`--surface-muted` deepened to #e7e9e4/#e5e7e2, `--line`/`--line-strong` opacity raised (0.08→0.1, 0.15→0.2) for more definition, `--muted` darkened slightly for better body-text contrast.
- **Redesigned the `welcome` and `login` account-gate stages** to match the polish already established on the `select` (avatar grid) screen, per explicit request to "continue" that direction: both now lead with a centered `.account-identity` block — the account's avatar at `avatar-lg` size with a gold-ring glow and a pop-in entrance animation, next to the greeting — instead of plain text. Centered the whole stage (`.account-stage-centered`) for a more deliberate, less form-like feel. Removed the redundant account `<select>` dropdown from the `login` stage: since `select` (the avatar grid) is now always the entry point when any account exists, `login` is only ever reached for the account already chosen there, so showing a dropdown of the same single context was dead weight — removed it along with the now-unused `onAccountChange` prop threaded through `App.tsx`.
- **Added tasteful, restrained animation in a few more places**, per request ("sans rendre le tout imbuvable" — kept deliberately light-touch, not on every element): a subtle press `scale(0.97)` feedback on all buttons; the budget-progress ring's `--progress` conic-gradient angle now animates smoothly on change via `@property --progress` (registered as an animatable `<angle>`) instead of snapping; error/update banners fade+slide in on mount; list rows and calendar days lift 1px and tint on hover; nav buttons nudge right 2px on hover.
- Validation: `npm run typecheck`, `npm run lint`, `npm test -- --runInBand`, `npm run build` all pass. Verified via the Vite dev server + `getBoundingClientRect()` that the account-card button width bug is fixed (button width now matches the card's content box exactly) and that the darkened light palette's `--page`/`--surface` tokens resolve correctly.

## [2026-08-23] - Finterest Change Session #35
- Files modified: `package.json`, `package-lock.json`, `DEV_CHANGES.md`.
- Discovered `package.json`'s `version` had never been bumped past the initial `0.1.0` across every prior session, while `PATCH_NOTES.md` had independently climbed to `v0.1.32` — these two version counters had silently diverged. Since `electron-updater` compares `package.json`'s version against published GitHub Release tags at runtime, publishing a release without reconciling them first would have made every future update check behave incorrectly. Bumped `package.json`/`package-lock.json` to `0.1.32` (`npm version 0.1.32 --no-git-tag-version`) to match the latest `PATCH_NOTES.md` entry, then rebuilt `install/windows/Finterest Setup 0.1.32.exe` so the shipped binary's embedded version metadata is correct.
- Merged `add-claude-md` into `main` via `gh pr merge` (PRs #4 and #5 — the accumulated feature work from Sessions #29-#34, then the version bump), installing GitHub CLI in the process (previously unavailable in this environment across many earlier attempts; it turned out to already be installed at `C:\Program Files\GitHub CLI\gh.exe`, just not yet on this shell's `PATH`).
- Published the project's **first GitHub Release**, `v0.1.32`, with `Finterest Setup 0.1.32.exe` and `latest.yml` (electron-updater's required metadata file) attached. Release notes cover the cumulative feature set shipped since `v0.1.0` (this repository never tagged/released `v0.1.1` through `v0.1.31`, so a narrow single-version changelog entry would have undersold what's actually in this first build) — sourced from `README.md`'s feature list, with an explicit caution that the Windows uninstaller's pre-uninstall backup mechanism (Session #28-29) still has not been verified in a real install/uninstall cycle.
- This also means the auto-updater is now "live" in the sense described to the user earlier: any Finterest install running a version older than `0.1.32` that checks GitHub Releases will now find this one.
...
- While investigating this, found a pre-existing Finterest install with an existing account and local data in `%APPDATA%\Finterest` on the dev machine, unrelated to this session's changes. Confirmed with the user it was disposable before any further local testing.
