...
- **`src/renderer/`** — React UI. `App.tsx` is a single top-level component covering account selection/login, the budgeting dashboard, and settings; `styles.css` holds four variable-driven palettes selected by `data-theme` on `<html>`: `nebula-dark` (the default, also on bare `:root`), `nebula-light`, and the frozen legacy `old-dark`/`old-light`; `theme.ts` owns the choice and resolves `system` to the Nebula pair. No component library or router is used.
